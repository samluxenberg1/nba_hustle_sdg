...
class TransformData:
    ...
    def engineered_features(self, output_dir: str) -> pd.DataFrame:
        ...
        self.df_proc['HOME_EST_POSS'] = self.df_proc['HOME_FGA'] + .44*self.df_proc['HOME_FTA'] + self.df_proc['HOME_TOV'] - self.df_proc['HOME_OREB']
        self.df_proc['AWAY_EST_POSS'] = self.df_proc['AWAY_FGA'] + .44*self.df_proc['AWAY_FTA'] + self.df_proc['AWAY_TOV'] - self.df_proc['AWAY_OREB']

        # Compute Offensive Ratings = PTS Scored / POSS x 100
        self.df_proc['HOME_EST_ORtg'] = self.df_proc['HOME_PTS']/self.df_proc['HOME_EST_POSS']*100
        self.df_proc['AWAY_EST_ORtg'] = self.df_proc['AWAY_PTS']/self.df_proc['AWAY_EST_POSS']*100

        # Compute Defensive Ratins = PTS Allowed / POSS x 100
        self.df_proc['HOME_EST_DRtg'] = self.df_proc['AWAY_EST_ORtg']
        self.df_proc['AWAY_EST_DRtg'] = self.df_proc['HOME_EST_ORtg'] 

        # Compute Net Ratings: 2 equivalent calculations
        # 1. HOME ORtg - HOME DRtg 
        # 2. HOME ORtg - AWAY ORtg
        self.df_proc['HOME_EST_NRtg'] = self.df_proc['HOME_EST_ORtg'] - self.df_proc['HOME_EST_DRtg']
        self.df_proc['AWAY_EST_NRtg'] = -self.df_proc['HOME_EST_NRtg']
